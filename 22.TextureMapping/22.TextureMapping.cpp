...
{
	auto vertexShaderCode = readFile("../Assets/Shaders/Texture.vert.spv");
	auto fragementShaderCode = readFile("../Assets/Shaders/Texture.frag.spv");
...
void HelloTriangleApplication::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
	...
	VkCheck(vkCreateImage(device, &imageCreateInfo, nullptr, &textureImage), "Failed to create image!");
	...
	VkCheck(vkAllocateMemory(device, &memoryAllocateInfo, nullptr, &textureImageMemory), "Failed to allocate image memory!");
